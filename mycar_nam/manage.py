...
Usage:
    manage.py (drive) [--model=<model>] [--js] [--type=(linear|categorical)] [--camera=(single|stereo)] [--meta=<key:value> ...] [--myconfig=<filename>] [--tub=<tub_path>]
...
from donkeycar.parts.route_logic import RouteManager
from donkeycar.parts.segmentation import RoadSegmentation
...
class StopController:
    '''
    Bộ điều khiển dừng xe có tính năng Active Braking (Phanh ngược).
    Khi nhận tín hiệu dừng:
    1. Trả về ga âm (ví dụ -0.5) trong khoảng thời gian ngắn để hãm quán tính.
    2. Sau đó trả về 0.0 để đứng yên.
    '''
    def __init__(self, brake_throttle=-0.2, brake_duration=0.5):
        self.brake_throttle = brake_throttle # Lực phanh (Số âm)
        self.brake_duration = brake_duration # Thời gian phanh (Giây)
        self.braking_start_time = 0
        self.is_braking = False
        self.has_stopped = False # Cờ đánh dấu đã hoàn thành quy trình dừng

    def run(self, throttle, stop_flag):
        # Nếu không có lệnh dừng, trả về ga bình thường từ Pilot/User
        if not stop_flag:
            self.is_braking = False
            self.has_stopped = False
            return throttle

        # Nếu có lệnh dừng (stop_flag = True)
        
        # 1. Nếu đã hoàn thành phanh xong xuôi -> Cắt ga hoàn toàn (0.0)
        if self.has_stopped:
            return 0.0

        # 2. Nếu mới bắt đầu nhận lệnh dừng -> Kích hoạt Timer
        if not self.is_braking:
            print(f"!!! ACTIVE BRAKING ENGAGED ({self.brake_throttle}) !!!")
            self.braking_start_time = time.time()
            self.is_braking = True
            return self.brake_throttle

        # 3. Nếu đang trong quá trình phanh
        if self.is_braking:
            elapsed = time.time() - self.braking_start_time
            
            if elapsed < self.brake_duration:
                # Vẫn trong thời gian phanh -> Giữ ga âm
                return self.brake_throttle
            else:
                # Hết giờ phanh -> Chuyển sang dừng hẳn
                print("--> Braking finished. Holding position.")
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
          camera_type='single', meta=[], tub_path_arg=None):
    """
    ...
    print(f"DEBUG: TRAIN_BEHAVIORS = {cfg.TRAIN_BEHAVIORS}")
    ...
    seg_part = RoadSegmentation(width=cfg.IMAGE_W, height=cfg.IMAGE_H, sensitivity=120, debug=True)
    V.add(seg_part, 
          inputs=['cam/image_array'], 
          outputs=['seg/area', 'seg/mask'], 
          threaded=True) # <-- Chạy song song
    
    if cfg.TRAIN_BEHAVIORS:
        print("Loading RouteManager Logic with D-PAD TOGGLE...")
        from donkeycar.parts.route_logic import RouteManager
        
        # 1. Khởi tạo
        route_logic = RouteManager(cfg)
        
        # 2. Class hỗ trợ xử lý D-Pad (Để chống lặp lệnh Toggle)
        class DPadHandler:
            def __init__(self, logic):
                self.logic = logic
                self.is_toggle_pressed = False # Cờ để kiểm soát nút Lên

            def handle_vert(self, val):
                if val > 0.5:
                    if not self.is_toggle_pressed:
                        self.logic.toggle_mode()
                        self.is_toggle_pressed = True
                
                elif val < -0.5:
                    self.logic.set_manual_normal()
                    self.is_toggle_pressed = False # Reset cờ
                
                else:
                    self.is_toggle_pressed = False # Reset cờ để lần sau bấm lại được

            def handle_horiz(self, val):
                if val < -0.5:
                    self.logic.set_manual_left()
                elif val > 0.5:
                    self.logic.set_manual_obstacle()

        dpad_handler = DPadHandler(route_logic)

        try:
            if ctr is not None:
                # ctr.set_axis_trigger('dpad_vert', dpad_handler.handle_vert)
                # ctr.set_axis_trigger('dpad_horiz', dpad_handler.handle_horiz)
                ctr.set_button_down_trigger('right_under_shoulder', route_logic.set_manual_right)
                
                ctr.set_button_down_trigger('left_under_shoulder', route_logic.set_manual_left)
                
                print("Mapping Configured:")
                print("  [0x138 - L2] -> Set Left_Turn (Quẹo trái)")
                print("  [0x139 - R2] -> Set Right_Turn (Quẹo phai)")
                print("Mapping Configured:")
                print("  [UP]    -> Toggle Auto/Manual")
                print("  [DOWN]  -> Set Normal (Đi thẳng)")
                print("  [LEFT]  -> Set Left (Rẽ trái)")
                print("  [RIGHT] -> Set Obstacle (Né vật)")
                
        except Exception as e:
            print(f"Warning: Could not map joystick axes. {e}")

        V.add(route_logic, 
              inputs=['cam/image_array', 'user/mode'], 
              outputs=["behavior/one_hot_state_array", "behavior/label", "user/stop_flag"])
    ...
    if model_path:
        ...
        #
        if cfg.TRAIN_BEHAVIORS:
            inputs.append("behavior/one_hot_state_array")
    ...
    V.add(stop_controller,
          inputs=['throttle', 'user/stop_flag'], # Lấy ga hiện tại và cờ dừng
          outputs=['throttle']) 
    ...
    #
    if tub_path_arg is not None:
        tub_path = os.path.join(cfg.DATA_PATH, tub_path_arg)
    elif cfg.AUTO_CREATE_NEW_TUB:
        tub_path = TubHandler(path=cfg.DATA_PATH).create_tub_path()
    else:
        tub_path = cfg.DATA_PATH

    print(f"--> Recording data to: {tub_path}")
...
def add_user_controller(V, cfg, use_joystick, input_image='ui/image_array'):
    """
    Add the web controller OR any other configured user input controller.
    """
    ctr = None
    
    # 1. KIỂM TRA JOYSTICK TRƯỚC
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        if cfg.CONTROLLER_TYPE == "pigpio_rc":
            ...
            V.add(ctr, 
                  ...
                  outputs=['user/steering', 'user/throttle', 'user/mode', 'recording'],
                  threaded=True)
        print("Using Joystick Controller.")

    # 2. NẾU KHÔNG CÓ JOYSTICK, DÙNG WEB CONTROLLER
    else:
...
if __name__ == '__main__':
    ...
    if args['drive']:
        ...
        drive(cfg, model_path=args['--model'], use_joystick=args['--js'],
              ...
              meta=args['--meta'], tub_path_arg=args.get('--tub'))
    elif args['train']:
